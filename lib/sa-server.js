 ...
 */
lib.generateNginxConf = function(config, callback) {
  var servers = {};
  var result = [];
...
//    });
    ...
    return sg.__each(config.fqdns, (server, next) => {
      ...
      // ----- Add content -----
      // Create root dir
      myMkdirp(server.root);

      result.push(`
    server {
      server_name ${server.fqdn};
      root ${server.root};
      access_log ${server.logfile} main;`);
    ...
    if (server.useHttps) {
      ...
      ssl_certificate ${server.certfile};
      ssl_certificate_key ${server.keyfile};
    ...
    if (server.requireClientCerts) {
      ...
      ssl_client_certificate ${server.clientCert};
    ...
    if (config.notnotnot) {
      ...
      myMkdirp(`${server.routes}`);
      touch.sync(`${server.routes}/routes`);
