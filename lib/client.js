...
verbose(2, (process.env.http_proxy? `Using proxy: ${process.env.http_proxy}` : `Not using proxy`) + ` for /clientStart`);
...
function POST(protocol, fqdn, pathname, query, body, callback) {
  ...
  if (true || (!isLocal && isProxied) || protocol.startsWith('https')) {
    ...
    return sg.__run([function(next) {
      ...
      return sg.exec('curl', ['-skLv', url, '-d', arg, '-H', 'Content-Type: application/json'], function(error, exitCode, stdoutChunks, stderrChunks, signal) {
        ...
        const result = sg.safeJSONParseQuiet(stdout);
