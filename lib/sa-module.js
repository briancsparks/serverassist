...
const nofriend = function(msg_) {
  var msg = msg_ || '';

  console.error(`--- !!!!!! Encountered unfriendly: ${msg}`);

  return true;
};

const unfriendlyClient = function(req, res, code) {
  const host = req.headers.host;

  // Empty host is bad
  if (!host)                              { return nofriend('empty host'); }

  const url = urlLib.parse(req.url);

  // Any request for PHP is bad
  if (url.pathname.match(/php/i))         { return nofriend(`PHP request ${url.pathname}`); }
  if (url.pathname.match(/^[/]pma/i))     { return nofriend(`PHP request ${url.pathname}`); }
  if (url.pathname.match(/^[/]admin/i))   { return nofriend(`Admin request ${url.pathname}`); }
  if (url.pathname.match(/^[/]mysql/i))   { return nofriend(`MySql request ${url.pathname}`); }

  // IP address host is bad
  if (host.match(/^[0-9:.]*$/))           { return nofriend(`IP address for host (${host}) ${url.pathname}`); }

  if (code === '_401')                    { return nofriend(`401 ${url.pathname}`); }
  if (code === '_403')                    { return nofriend(`403 ${url.pathname}`); }
...
_.each(sg, (fn, name) => {
  if (name.match(/^_[0-9][0-9][0-9]$/)) {
    lib[name] = function(req, res, content, debugInfo_, headers) {
      ...
      const isUnfriendly = unfriendlyClient(req, res, name);
      ...
      if (name === '_403' || name === '_401' || isUnfriendly) {
        debugInfo = null;
      }
