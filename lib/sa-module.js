...
lib.redirectToService = function(req, res, name, err, location, rewritten_) {
  ...
  const locationUrl       = urlLib.parse(location || '');
  const internalPathname  = locationUrl.pathname || '/';
  const internalEndpoint  = location.replace(/^(http|https):[/][/]/i, '').replace(new RegExp(internalPathname+'$'), '');
  const internalEndParts  = _.compact(internalPathname.split('/'));
  ...
  while (internalEndParts.length > 0) {
    if (internalEndParts[0] === '_X') {
      rewritten = _.rest(_.compact(rewritten.split('/'))).join('/');
      internalEndParts.shift();
    } else {
      break;
    }
  }

  const redir             = normlz(`/rpxi/${req.method}/${internalEndpoint}/${internalEndParts.join('/')}/${rewritten}`);
...
lib.decodeRsvr = function(rsvr_) {
  ...
  if (rsvr) {
    ...
    rsvr = rsvr.replace(/^hq/i, '');

    if (rsvr.startsWith('qa'))            { requestedStack = 'test'; }
    else if (rsvr.startsWith('test'))     { requestedStack = 'test'; }
    else if (rsvr.startsWith('pub'))      { requestedStack = 'pub'; }
    else if (rsvr.startsWith('prod'))     { requestedStack = 'pub'; }

    if (rsvr.match(/next$/i))             { requestedState = 'next'; }
  }

  return [requestedStack, requestedState];
};
