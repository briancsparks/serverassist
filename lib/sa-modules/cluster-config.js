...
lib.configuration = function(argv, context, callback) {
  ...
  return MongoClient.connect(mongoHost, (err, db) => {
    ...
    return sg.__runll([function(next) {
      ...
      // ---------- projects ----------
      return projectsDb.find({active:{$ne:false},deployStyle:{$exists:true}}, {_id:0}).toArray((err, rawProjectRecords) => {
    ...
    // ---------- mini-projects ----------
      ...
      return projectsDb.find({active:{$ne:false},deployStyle:{$exists:false}}, {_id:0}).toArray((err, rawProjectRecords) => {
        if (err) { return sg.die(err, callback, 'configuration.each-project'); }

        projectRecords      = sg.reduce(rawProjectRecords, projectRecords, (m, project) => { return sg.kv(m, project.projectId, project); });
        miniProjectRecords  = sg.reduce(rawProjectRecords, {}, (m, project) => { return sg.kv(m, project.projectId, project); });
        return next();
      });
    ...
    // ---------- Build Config ----------
      ...
      return sg.__run([function(next) {
        ...
        return callback(null, {db:{projectRecords, miniProjectRecords, partnerRecords, stackRecords, instanceRecords, appRecords, appprjRecords}, result, myStack: subStacks[`${myColor}-${myStack}`]});
      });
    });
  });
};
