...
lib.loadHttpServer = function(name, options_, callback) {
  ...
  const dbHostname            = options.dbHostname;
  ...
  const mongoHost             = helpers.mongoHost(dbName, dbHostname);
  ...
  return MongoClient.connect(mongoHost, (err, db) => {
     ...
     */
      ...
      server = http.createServer((req, res) => {
        ...
        // Remember headers that get set on the response. headers2 is a table that can
        // be looked-up much easier
        ...
        // ---------- helpers ----------
        const logContentLength = function() {
          return res.serverassist.headers2.logContentlength || '-stream-';
        };
        ...
        res.end = function() {
          ...
          setOnn(res, ['serverassist', 'msg', lengthIndex],       sg.pad(logContentLength(), 10, ' '));
